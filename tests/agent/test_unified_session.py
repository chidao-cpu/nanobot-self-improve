...
class TestConsolidationUnaffectedByUnifiedSession:
    ...
    async def test_consolidation_triggers_when_over_budget_unified_key(self):
        """Consolidator.maybe_consolidate_by_tokens is deprecated (no-op).
        ContextCompressor handles inline compression instead."""
        from nanobot.agent.memory import Consolidator, MemoryStore
        ...
        # maybe_consolidate_by_tokens is now a no-op — nothing should be called
