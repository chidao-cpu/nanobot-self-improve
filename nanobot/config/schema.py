...
class CuratorSchemaConfig(Base):
    ...
    skill_nudge_interval: int = Field(default=4, ge=0)  # Tool iterations before skill nudge (0=disabled)
...
class AgentDefaults(Base):
    ...
    curator: CuratorSchemaConfig = Field(default_factory=CuratorSchemaConfig)
...
class ToolsConfig(Base):
    ...
    excel_analyzer: ExcelAnalyzerConfig = Field(
        default_factory=lambda: _lazy_default("nanobot.agent.tools.excel_analyzer", "ExcelAnalyzerConfig"),
    )
    sql_query: SQLQueryConfig = Field(
        default_factory=lambda: _lazy_default("nanobot.agent.tools.sql_query", "SQLQueryConfig"),
    )
    code_sandbox: CodeSandboxConfig = Field(
        default_factory=lambda: _lazy_default("nanobot.agent.tools.code_sandbox", "CodeSandboxConfig"),
    )
    memory_tool: Any = Field(
        default_factory=lambda: _lazy_default("nanobot.agent.tools.memory_tool", "MemoryToolConfig"),
    )
    skill_manager: Any = Field(
        default_factory=lambda: _lazy_default("nanobot.agent.tools.skill_manager", "SkillManagerConfig"),
    )
    skills: Any = Field(
        default_factory=lambda: _lazy_default("nanobot.agent.tools.skills_tool", "SkillsToolConfig"),
    )
...
class ToolPermissionsConfig(Base):
    """Tool-level permission control configuration."""

    enabled: bool = False
    permissions: dict[str, Any] = Field(default_factory=dict)


class FileAccessControlConfig(Base):
    """File whitelist/blacklist access control configuration."""
...
class AmbiguityDetectionConfig(Base):
    """歧义检测配置"""
    ...
    max_pause_per_turn: int = Field(default=3, ge=0, le=10)
...
class Config(BaseSettings):
    ...
    ambiguity_detection: AmbiguityDetectionConfig = Field(default_factory=AmbiguityDetectionConfig)
    tool_permissions: ToolPermissionsConfig = Field(default_factory=ToolPermissionsConfig)
    file_access_control: FileAccessControlConfig = Field(default_factory=FileAccessControlConfig)
...
def _resolve_tool_config_refs() -> None:
    ...
    from nanobot.agent.tools.cli_apps import CliAppsToolConfig
    from nanobot.agent.tools.code_sandbox import CodeSandboxConfig
    from nanobot.agent.tools.excel_analyzer import ExcelAnalyzerConfig
    from nanobot.agent.tools.filesystem import FileToolsConfig
    from nanobot.agent.tools.image_generation import ImageGenerationToolConfig
    from nanobot.agent.tools.self import MyToolConfig
    from nanobot.agent.tools.shell import ExecToolConfig
    from nanobot.agent.tools.sql_query import SQLQueryConfig
    from nanobot.agent.tools.web import WebFetchConfig, WebSearchConfig, WebToolsConfig

    # Re-export into this module's namespace
    mod = sys.modules[__name__]
    mod.CodeSandboxConfig = CodeSandboxConfig  # type: ignore[attr-defined]
    mod.ExecToolConfig = ExecToolConfig  # type: ignore[attr-defined]
    mod.ExcelAnalyzerConfig = ExcelAnalyzerConfig  # type: ignore[attr-defined]
    mod.FileToolsConfig = FileToolsConfig  # type: ignore[attr-defined]
    mod.CliAppsToolConfig = CliAppsToolConfig  # type: ignore[attr-defined]
    mod.SQLQueryConfig = SQLQueryConfig  # type: ignore[attr-defined]
