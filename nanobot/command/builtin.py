...
BUILTIN_COMMAND_SPECS: tuple[BuiltinCommandSpec, ...] = (
    ...
    BuiltinCommandSpec(
        "/learn",
        "Learn a skill",
        "Research a topic and save it as a reusable skill.",
        "graduation-cap",
        "<what to learn>",
        lifecycle="agent_turn_with_args",
        accepts_args=True,
...
async def cmd_dream(ctx: CommandContext) -> OutboundMessage:
    ...
    async def _run_dream():
        ...
        t0 = time.monotonic()
        try:
            # Use session-based Dream prompt (reads from session messages)
            result = store.build_dream_prompt_from_sessions(loop.sessions)
            if result is None:
                ...
                return
            prompt, cursor_timestamp = result
            ...
            if completed:
                store.set_last_dream_cursor(cursor_timestamp)
                if diff_body:
...
def _format_dream_no_input_message() -> str:
    return "\n".join([
        ...
        "",
        "Dream reads uncompressed messages directly from active sessions.",
        (
            "Short or recent chats may not have enough uncompressed messages "
            "for Dream to process. Messages are available for Dream after they "
            "accumulate beyond the compression boundary."
        ),
        "",
        "Next steps:",
        "- Have more conversations so uncompressed messages accumulate.",
        "- Enable `agents.defaults.idleCompactAfterMinutes` so completed chats trigger compression automatically.",
...
async def cmd_learn(ctx: CommandContext) -> OutboundMessage | None:
    """Learn a new skill from a user request.

    No args: show usage (side-channel).
    With args: inject a learn prompt and enter the normal agent turn.
    """
    ...
    if not topic:
        return OutboundMessage(
            ...
            content=(
                ...
                "Research a topic and save it as a reusable skill.\n"
                "Example: `/learn how to configure nginx reverse proxy`"
            ),
            metadata={**dict(ctx.msg.metadata or {}), "render_as": "text"},
        )
    if ctx.session is None:
        return OutboundMessage(
            ...
            content=(
                "A task is already running for this chat. "
                "Use `/stop` first, then send `/learn <topic>` again."
            ),
            metadata={**dict(ctx.msg.metadata or {}), "render_as": "text"},
        )

    # Load skill-creator standards for authoring guidance
    from nanobot.agent.learn_prompt import build_learn_prompt

    skill_creator_standards = ""
    try:
        content = ctx.loop.context.skills.load_skill("skill-creator")
        if content:
            skill_creator_standards = content
    except Exception:
        pass

    prompt = build_learn_prompt(topic, skill_creator_standards)

    ctx.msg.metadata = {
        **dict(ctx.msg.metadata or {}),
        "original_command": "/learn",
        "original_content": ctx.raw,
    }
    ctx.msg.content = prompt
    return None
