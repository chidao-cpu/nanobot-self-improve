...
from nanobot.agent.hooks.ambiguity_detection import AmbiguityPauseException
...
class AgentRunResult:
    ...
    tool_iterations: int = 0  # Number of iterations that executed tools
...
class AgentRunner:
    ...
    async def _handle_ambiguity_pause(
        self,
        spec: AgentRunSpec,
        exc: AmbiguityPauseException,
        messages: list[dict[str, Any]],
        iteration: int,
    ) -> bool:
        """Handle ambiguity pause: send question and wait for user response.
        
        Returns True if user responded and we should continue, False otherwise.
        """
        logger.info(
            "Ambiguity detected at iteration {}: {}",
            iteration, exc.question,
        )
        
        # Send ambiguity question to user via progress callback
        if spec.progress_callback:
            await spec.progress_callback(
                f"⚠️ **检测到歧义，需要您的澄清**\n\n"
                f"**问题**：{exc.question}\n\n"
                f"请回复您的澄清，或输入 `/skip` 跳过此问题。"
            )
        
        # Wait for user response via injection callback
        if spec.injection_callback:
            try:
                injections = await spec.injection_callback(limit=1)
                if injections:
                    user_response = injections[0]
                    if isinstance(user_response, dict):
                        user_text = user_response.get("content", str(user_response))
                    else:
                        user_text = str(user_response)
                    
                    # Check for /skip command
                    if user_text.strip() == "/skip":
                        logger.info("User skipped ambiguity question")
                        messages.append({
                            "role": "user",
                            "content": "[用户选择跳过，请自行决策]",
                        })
                    else:
                        messages.append({
                            "role": "user",
                            "content": f"[用户澄清] {user_text}",
                        })
                        logger.info("User responded to ambiguity: {}", user_text[:100])
                    return True
            except Exception:
                logger.exception("Failed to get ambiguity response")
        
        # If no injection callback or no response, log warning
        logger.warning("No user response to ambiguity, continuing execution")
        return False
    ...
    async def run(self, spec: AgentRunSpec) -> AgentRunResult:
        ...
        context = AgentRunHookContext(messages=deepcopy(messages), session_key=spec.session_key)

        try:
            await hook.before_run(context)
            # Sync any modifications made by hooks (e.g., dependency injection) back to messages
    ...
    async def _run_core(
        ...
        tool_iterations = 0  # Count iterations that executed tools
        ...
        for iteration in range(spec.max_iterations):
            try:
                ...
                if response.should_execute_tools:
                    ...
                    tool_iterations += 1  # Count this tool-executing iteration
                ...
                await hook.after_iteration(context)
                break
            except AmbiguityPauseException as exc:
                handled = await self._handle_ambiguity_pause(spec, exc, messages, iteration)
                if handled:
                    continue
                break
        else:
