...
class ExecTool(Tool):
    ...
    def __init__(
        ...
        self.deny_patterns = (deny_patterns or []) + [
            ...
            # .dream_cursor is managed internally; direct writes corrupt the cursor.
            r">>?\s*\S*\.dream_cursor",            # > / >> redirect
            r"\btee\b[^|;&<>]*\.dream_cursor",     # tee / tee -a
            r"\b(?:cp|mv)\b(?:\s+[^\s|;&<>]+)+\s+\S*\.dream_cursor",  # cp/mv target
            r"\bdd\b[^|;&<>]*\bof=\S*\.dream_cursor",  # dd of=
            r"\bsed\s+-i[^|;&<>]*\.dream_cursor",  # sed -i
        ]
