...
from nanobot.agent.background_review import BackgroundReviewer
...
class AgentLoop:
    ...
    _MEMORY_NUDGE_INTERVAL = 10  # Remind agent to save memories every N turns
    ...
    def __init__(
        ...
        curator_promote_use_count: int = 5,
        ...
        self.curator_consolidate = curator_consolidate
        self.curator_promote_use_count = curator_promote_use_count
        ...
        # BackgroundReviewer needs memory store + optional skill dependencies
        # Skill dependencies are wired after _register_default_tools creates them
        self._background_reviewer = BackgroundReviewer(
            workspace=self.workspace,
            memory_entry_store=self._memory_entry_store,
            skills_loader=self.context.skills if hasattr(self.context, 'skills') else None,
            skill_usage_store=self._skill_usage_store,
            skill_nudge_interval=skill_nudge_interval,
        )
        self._runtime_vars: dict[str, Any] = {}
        self._current_iteration: int = 0
        self._turns_since_memory: int = 0  # Nudge counter for memory reminders
    ...
    def from_config(
        ...
        **extra: Any,
        ...
        # Build extra hooks from config
        config_hooks: list[AgentHook] = []
        if config.ambiguity_detection.enabled:
            from nanobot.agent.hooks.ambiguity_detection import AmbiguityInterruptionHook
            config_hooks.append(AmbiguityInterruptionHook(
                detect_reasoning_uncertainty=config.ambiguity_detection.detect_reasoning_uncertainty,
                detect_tool_parameter_ambiguity=config.ambiguity_detection.detect_tool_parameter_ambiguity,
                detect_tool_result_anomaly=config.ambiguity_detection.detect_tool_result_anomaly,
                detect_low_confidence_answer=config.ambiguity_detection.detect_low_confidence_answer,
                max_pause_per_turn=config.ambiguity_detection.max_pause_per_turn,
            ))
        
        # Add tool permission hook if enabled
        if config.tool_permissions.enabled:
            from nanobot.agent.hooks.tool_permission import ToolPermissionHook
            from nanobot.security.tool_permissions import ToolPermissionManager
            permission_manager = ToolPermissionManager(config.tool_permissions.permissions)
            config_hooks.append(ToolPermissionHook(permission_manager))
        
        # Add file access control hook if enabled
        if config.file_access_control.enabled:
            from nanobot.agent.hooks.file_access import FileAccessHook
            from nanobot.security.file_whitelist import FileAccessControl, FileWhitelist, FileBlacklist
            whitelist = FileWhitelist(config.file_access_control.whitelist)
            blacklist = FileBlacklist(config.file_access_control.blacklist)
            file_access_control = FileAccessControl(whitelist, blacklist)
            config_hooks.append(FileAccessHook(file_access_control))
        
        # Merge config hooks with any caller-supplied hooks
        existing_hooks = extra.pop("hooks", None) or []
        merged_hooks = config_hooks + existing_hooks
        return cls(
            bus=bus,
            provider=provider,
            ...
            curator_consolidate=defaults.curator.consolidate,
            curator_promote_use_count=defaults.curator.promote_use_count,
            skill_nudge_interval=defaults.curator.skill_nudge_interval,
            provider_snapshot_loader=provider_snapshot_loader,
            preset_snapshot_loader=preset_snapshot_loader,
            hooks=merged_hooks,
            **extra,
    ...
    def _register_default_tools(
        ...
        *,
        ...
        # MemoryTool needs MemoryEntryStore reference — manual registration
        if self.tools_config.memory_tool.enable:
            from nanobot.agent.tools.memory_tool import MemoryEntryStore, MemoryTool

            memory_file = self.workspace / "memory" / "MEMORY.md"
            char_limit = self.tools_config.memory_tool.memory_char_limit
            self._memory_entry_store = MemoryEntryStore(
                memory_file=memory_file,
                char_limit=char_limit,
            )
            self._memory_entry_store.load_from_disk()
            memory_tool = MemoryTool(store=self._memory_entry_store)
            self.tools.register(memory_tool)
            registered.append("memory")
        else:
            self._memory_entry_store = None

        # SkillManagerTool needs workspace + usage_store reference — manual registration
        if self.tools_config.skill_manager.enable:
            from nanobot.agent.skill_usage import SkillUsageStore
            from nanobot.agent.tools.skill_manager import SkillManagerTool

            self._skill_usage_store = SkillUsageStore(self.workspace)
            # Wire usage tracking into the skills loader so every load_skill
            # call records a use event automatically.
            self.context.skills.set_usage_store(self._skill_usage_store)
            skill_manager = SkillManagerTool(
                workspace=self.workspace,
                usage_store=self._skill_usage_store,
            )
            self.tools.register(skill_manager)
            registered.append("skill_manager")
        else:
            self._skill_usage_store = None

        # SkillsTool needs SkillsLoader + SkillUsageStore reference — manual registration
        if self.tools_config.skills.enable and self._skill_usage_store is not None:
            ...
            self.tools.register(skills_tool)
    ...
    def _build_initial_messages(self, ctx: TurnContext) -> list[dict[str, Any]]:
        ...
        # Increment nudge counter for user turns (not dream/subagent)
        should_nudge = False
        ...
        # Inject memory nudge if threshold reached
        if should_nudge and messages:
            nudge = (
                "\n\n[REMINDER: You have not saved any memories recently. "
                "If the conversation contained important facts, preferences, "
                "or corrections, use the memory tool now.]"
            )
            # Append to the first system message
            if messages[0].get("role") == "system":
                messages[0]["content"] = messages[0]["content"] + nudge
    ...
    async def _run_agent_loop(
    ...
    ) -> tuple[str | None, list[str], list[dict[str, Any]], str, bool, int]:
        ...
        *on_stream_end(resuming, merge_next)*: called when a streaming session finishes.
        ...
        Returns (final_content, tools_used, messages, stop_reason, had_injections, tool_iterations).
        """
        ...
        # Reset the memory-nudge counter when the agent actually used the memory
        # tool during this turn so the reminder interval restarts.
        if "memory" in result.tools_used:
            self._turns_since_memory = 0
        ...
        return result.final_content, result.tools_used, result.messages, result.stop_reason, result.had_injections, result.tool_iterations
    ...
    def _maybe_schedule_curator(self) -> None:
        """Check if idle-triggered Curator should run (Hermes-style).

        Runs Curator in background if:
        1. At least 7 days since last run
        2. Agent has been idle for at least 30 minutes
        """
        ...
        workspace = self.workspace
        last_activity = getattr(self, "_last_activity_at", 0.0)

        if not should_run_curator(workspace, last_activity):
            return

        async def _run_curator() -> None:
            try:
                usage_store = SkillUsageStore(workspace)
                # Get runtime from the most recent session for LLM consolidation
                runtime = None
                if self.sessions.sessions:
                    # Use the first available session's runtime
                ...
                curator = Curator(
                    ...
                    config=CuratorConfig(
                        consolidate=self.curator_consolidate,
                        promote_use_count=self.curator_promote_use_count,
                    ),
                )
                result = await curator.run(dry_run=False, runtime=runtime)
                transitions = result.get("transitions", [])
                always_changes = result.get("dynamic_always_changes", [])
                llm_result = result.get("llm_consolidation")
                
                if transitions:
                    logger.info(
                        "Curator: applied {} state transition(s): {}",
                        len(transitions),
                        transitions,
                    )
                if always_changes:
                    logger.info(
                        "Curator: applied {} dynamic_always change(s): {}",
                        len(always_changes),
                        always_changes,
                    )
                if llm_result:
                    ...
                    if tools_used:
                        logger.info(
                            "Curator LLM consolidation: {} tool(s) used - {}",
                            len(tools_used),
                            summary[:200] if summary else "no summary",
                        )
                    elif summary and "skipped" not in summary:
                        logger.info("Curator LLM consolidation: {}", summary[:200])
                
                if not transitions and not always_changes and not llm_result:
                    logger.debug("Curator: no changes needed")
                mark_curator_run(workspace)
            except Exception:
                logger.exception("Idle-triggered Curator failed")

        self.schedule_background(_run_curator())
    ...
    async def _build_turn(self, ctx: TurnContext) -> None:
        ...
        if not ctx.ephemeral:
            # Use new Hermes-style inline compression instead of old Consolidator
            await self.context_compressor.maybe_compress(
                session,
                runtime=runtime,
            )
    ...
    async def _run_turn(self, ctx: TurnContext) -> None:
        ...
        final_content, tools_used, all_msgs, stop_reason, had_injections, tool_iterations = result
        ctx.final_content = final_content
        ctx.all_messages = all_msgs
        ctx.stop_reason = stop_reason
        ctx.tools_used = tools_used
        ctx.had_injections = had_injections
        ctx.tool_iterations = tool_iterations
        if ctx.kind is TurnKind.USER:
    ...
    async def _persist_turn(self, ctx: TurnContext) -> None:
        ...
        if not ctx.ephemeral:
            ...
            # Background memory review: fork agent reviews conversation
            # and proactively saves important facts to MEMORY.md.
            # Skip if the foreground agent already used memory/skill tools
            # this turn — avoids double-writing the same facts.
            if ctx.kind is TurnKind.USER and not (
                "memory" in ctx.tools_used
                or "skill_manage" in ctx.tools_used
            ):
                self.schedule_background(
                    self._background_reviewer.maybe_review(
                        session,
                        runtime=runtime,
                        tool_iterations=ctx.tool_iterations,
                    )
                )

            # Idle-triggered Curator (Hermes-style): check if skill lifecycle
            # maintenance should run. Only triggers when enough time has passed
            # since last run AND the agent has been idle long enough.
            self._maybe_schedule_curator()
        self._clear_pending_user_turn(session)
