...
class AutoCompact:
    ...
    def __init__(self, sessions: SessionManager, compressor: ContextCompressor,
    ...
    async def _archive(self, key: str, *, runtime: LLMRuntime) -> None:
        ...
        try:
            session = self.sessions.get_or_create(key)
            # Use new ContextCompressor instead of old Consolidator
            compressed = await self.compressor.compress_session(
                session,
                runtime=runtime,
            )
            if compressed:
                logger.info("Auto-compact: compressed idle session {}", key)
        except Exception:
