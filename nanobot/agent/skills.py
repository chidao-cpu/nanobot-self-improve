...
class SkillsLoader:
    ...
    def __init__(self, workspace: Path, builtin_skills_dir: Path | None = None, disabled_skills: set[str] | None = None):
        ...
        self._usage_store: Any = None  # Optional SkillUsageStore for tracking

    def set_usage_store(self, store: Any) -> None:
        """Attach a SkillUsageStore for automatic usage tracking."""
    ...
    def list_skills(self, filter_unavailable: bool = True) -> list[dict[str, str]]:
        ...
        # Filter out archived skills (state == "archived" in SkillUsageStore)
        if self._usage_store is not None:
            try:
                archived = {
                    rec.name
                    for rec in self._usage_store.list_all()
    ...
    def load_skills_for_context(self, skill_names: list[str]) -> str:
        ...
        for name in skill_names:
            ...
            if markdown:
                ...
                # Track actual usage: skill injected into conversation context
                if self._usage_store is not None:
                    try:
                        self._usage_store.record_use(name)
                    except Exception:
                        pass
        return "\n\n---\n\n".join(parts)
    ...
    def get_always_skills(self) -> list[str]:
        """Get skills that should be loaded into every conversation.

        Includes both:
        - Static always: skills with always:true in YAML frontmatter
        - Dynamic always: skills auto-promoted by Curator based on usage
        """
        ...
        for entry in available:
            name = entry["name"]
            # Static always: from frontmatter
        ...
        # Dynamic always: from usage store
        if self._usage_store is not None:
            try:
                for name in self._usage_store.get_dynamic_always_skills():
