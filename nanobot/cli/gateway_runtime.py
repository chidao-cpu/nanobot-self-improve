...
def _advance_dream_cursor_if_behind(memory: Any) -> None:
    """When Dream is disabled, set the cursor to now so future Dream runs skip old sessions."""
    memory.set_last_dream_cursor()
    ...
    *,
    ...
    async def on_cron_job(job: CronJob) -> str | None:
        ...
        if job.name == "dream":
            ...
            diff_body = ""
            try:
                result = store.build_dream_prompt_from_sessions(agent.sessions)
                if result is None:
        ...
        # Curator is a system job that applies skill state transitions.
        if job.name == "curator":
            try:
                ...
                usage_store = SkillUsageStore(config.workspace_path)
                # Get runtime from agent for LLM consolidation
                runtime = None
                if agent.sessions.sessions:
                    for session in agent.sessions.sessions.values():
                ...
                curator = Curator(
                    ...
                    config=CuratorConfig(
                        consolidate=agent.curator_consolidate,
                        promote_use_count=agent.curator_promote_use_count,
                    ),
                )
                ...
                # Sync state so idle-triggered path knows cron ran
                mark_curator_run(config.workspace_path)
            except Exception:
    ...
    # Register Curator system job (idempotent on restart)
    # Weekly fallback: idle-triggered Curator runs more frequently, but cron ensures
    # it runs at least once per week even if the agent is never idle.
    curator_interval_ms = 7 * 24 * 60 * 60 * 1000  # 7 days
    cron.register_system_job(CronJob(
        id="curator",
        name="curator",
        schedule=CronSchedule(
            kind="every",
            every_ms=curator_interval_ms,
            tz=config.agents.defaults.timezone,
        ),
        payload=CronPayload(kind="system_event"),
    ))
    console.print("[green]✓[/green] Curator: idle-triggered + weekly fallback")
